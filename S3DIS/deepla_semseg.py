...
class Stage(nn.Module):
    ...
    def forward(self, x, xyz, prev_knn, indices, pts_list, sub_spa=None, sub_sem=None):
        ...
        if self.training:
            # spa
            spa_info = xyz[knn.squeeze(0)] - xyz.unsqueeze(1)
            spa_info.mul_(self.cor_std)
            feat_info = self.cor_head(x)
            feat_info = feat_info[knn.squeeze(0)] - feat_info.unsqueeze(1)
            closs = F.mse_loss(feat_info, spa_info)
            sub_spa = sub_spa + closs if sub_spa is not None else closs

            # sem
            sem = self.sem_sup(torch.max(spa_info, dim=1, keepdim=False)[0])
